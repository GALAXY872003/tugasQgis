...
<div class="container">
    <h1 class="page-title">Edit Usaha Desa</h1>

    <div class="form-container">
        <form action="{{ route('usaha_desa.update', $usahaDesa->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="form-control @error('nama_usaha') is-invalid @enderror" 
                            id="nama_usaha" name="nama_usaha" value="{{ old('nama_usaha', $usahaDesa->nama_usaha) }}" required>
                        @error('nama_usaha')
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="form-control @error('jenis_usaha') is-invalid @enderror" 
                            id="jenis_usaha" name="jenis_usaha" value="{{ old('jenis_usaha', $usahaDesa->jenis_usaha) }}" required>
                        @error('jenis_usaha')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <textarea class="form-control @error('deskripsi') is-invalid @enderror" 
                            id="deskripsi" name="deskripsi" rows="3">{{ old('deskripsi', $usahaDesa->deskripsi) }}</textarea>
                        @error('deskripsi')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-select @error('penduduk_id') is-invalid @enderror" 
                            ...
                            @foreach($penduduk as $p)
                                <option value="{{ $p->id }}" {{ old('penduduk_id', $usahaDesa->penduduk_id) == $p->id ? 'selected' : '' }}>
                                    {{ $p->nama_penduduk }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control @error('koordinat') is-invalid @enderror" 
                                id="koordinat" name="koordinat" value="{{ old('koordinat', $usahaDesa->koordinat) }}" 
                                readonly required>
                            <button type="button" class="btn btn-primary" onclick="centerMapOnJepara()">
...
<script>
    ...
    // Inisialisasi peta
    function initMap() {
        // Ambil koordinat dari input jika ada
        var koordinatInput = document.getElementById('koordinat').value;
        var initialLat = defaultLat;
        var initialLng = defaultLng;

        if (koordinatInput) {
            var coords = koordinatInput.split(',');
            initialLat = parseFloat(coords[0]);
            initialLng = parseFloat(coords[1]);
        }

        map = L.map('map').setView([initialLat, initialLng], 12); // Zoom level 12 untuk area kabupaten
